    ...
    try {
      ...
      switch (method) {
        case "GET": {
          ...
          return res.redirect((redirect_to as string) || "/");
        }
        case "POST": {
          ...
          await sendGrid.send({
            ...
            dynamicTemplateData: {
              link: `${proto}://${store}.${process.env.NEXT_PUBLIC_DOMAIN}/api/_store/${store}/login/email?token=${seal}&redirect_to=${redirectTo}`,
            },
          });
