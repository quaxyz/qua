...
const LOG_TAG = "[admin-email-setup]";
    ...
    try {
      ...
      switch (method) {
        case "GET": {
          ...
          if (!user) {
            console.error(LOG_TAG, "no user found from token");
            return res.status(404).send({ error: "not found" });
          }
          ...
          return res.redirect(`/setup/details`);
        }
        case "POST": {
          const { email } = body;
          ...
          await sendGrid.send({
            ...
            dynamicTemplateData: {
              link: `${proto}://admin.${process.env.NEXT_PUBLIC_DOMAIN}/api/admin/setup/email?token=${seal}`,
            },
          });
