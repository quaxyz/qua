...
const DashboardLayout: React.FC = ({ children }) => {
  ...
  return (
    <Grid
      ...
      minH="100vh"
    >
      <chakra.aside gridArea="sidebar" bg="#000000" px={4} py={8} display={{ base: "none", md: "block" }}>
      ...
      <chakra.header gridArea="topbar" bg="#000000" px={5} display={{ base: "block", md: "none" }}>
        <Stack direction="row" alignItems="center" justify="space-between" h="100%" w="100%">
      ...
      <chakra.aside gridArea="bottombar" bg="#000000" display={{ base: "block", md: "none" }}>
        <Stack direction="row" alignItems="center" justify="space-around" h="100%" w="100%">
          {navLinks.map((NavLink, idx) => (
            <NextLink key={idx} href={NavLink.url} passHref>
              <Link
                borderBottom="none"
                color="#FFF"
                {...(router.asPath.includes(NavLink.url) ? { textDecoration: "underline" } : {})}
              >
                <Stack spacing={2} align="center">
                  <Icon
                    boxSize={5}
                    as={(props) => (
                      <NavLink.icon {...(router.asPath.includes(NavLink.url) ? { set: "bold" } : {})} {...props} />
                    )}
                  />
                  <Text fontWeight="normal" color="inherit" fontSize="xs" as="span">
                    {NavLink.name}
                  </Text>
                </Stack>
              </Link>
