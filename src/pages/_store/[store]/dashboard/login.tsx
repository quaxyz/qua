...
const useGoogleAuth = () => {
  ...
  const url = React.useMemo(() => {
    ...
    const params = new URLSearchParams({
      ...
      state: JSON.stringify({
        ...
        path: global.location?.href, // path to send the params back to
      }),
    }).toString();
  ...
  const googleAuthMutation = useMutation(
    async () => {
      ...
      await Api().post("/dashboard/login/google", {
        code: data.code,
      });
    },
    {
      onSuccess: async () => {
        await queryClient.invalidateQueries(["store-user"]);
      },
...
const useWalletAuth = () => {
  ...
  const walletAuthMutation = useMutation(
    async (account: string) => {
      ...
      await Api().post("/dashboard/login/wallet", {
        address: account,
        sig,
        message,
      });
